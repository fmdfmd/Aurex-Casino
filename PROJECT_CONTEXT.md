...
### Резервные VPS (Inferno Solutions — Нидерланды) — куплены 28.02.2026

Все 3 сервера настроены: nginx → Railway (`xtjxpx6j.up.railway.app`), ufw (22/80/443), готовы к мгновенному переключению. Чтобы активировать — поменять DNS A-запись домена на нужный IP в Cloudflare, SSL выпустить через certbot.

| # | IP | Пароль | Статус |
|---|-----|--------|--------|
| 1 | **185.115.33.67** | `956H64Fpks` | ✅ nginx запущен |
| 2 | **185.115.33.66** | `8AMGoHnfGq` | ✅ nginx запущен |
| 3 | **37.1.202.68** | `H29sD7j2CT` | ✅ nginx запущен |

**Как переключить домен на резервный VPS:**
1. Cloudflare DNS → A-запись домена → поменять IP
2. SSH на новый сервер
3. `certbot --nginx -d ИМЯ_ДОМЕНА` (выпустить SSL)
4. `systemctl reload nginx`
5. Готово — сайт работает через новый VPS
